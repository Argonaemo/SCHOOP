...
const EbookEdit = () => {
  ...
  const { isEditBook, ebookData } = useContext(BookContext);
  ...
  useEffect(() => {
    if (getLocalStorageSession("adminToken")) {
      if (isEditBook) {
        setTitle(ebookData.judul);
        setSynopsis(ebookData.sinopsis);
        setStock(ebookData.stok);
        setAuthor(ebookData.penulis);
        setPublisher(ebookData.penerbit);
        setPages(ebookData.jumlahHalaman);
        //   setFile("");
        setYear(ebookData.tahunTerbit);
        setCategory(ebookData.categories);
      } else {
        navigate("/ebookoview");
      }
    } else {
  ...
  const updateHandler = async (ev) => {
    ...
    if (file !== null && pdf !== null) {
      ...
      const newUrlPdf = await setFirestoreStorage(pdf, "ebook/");
      await deleteFirestoreStorage(ebookData.image, "book/");
      await deleteFirestoreStorage(ebookData.ebook, "ebook/");
      const userDoc = doc(db, "buku", ebookData.id);
